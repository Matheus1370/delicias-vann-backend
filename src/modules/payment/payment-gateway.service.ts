...
export interface CardCheckoutOutput {
  transacaoId: string;
  checkoutUrl: string;
  raw: any;
}
...
export class PaymentGatewayService {
  ...
  async createCardCheckout(input: CardCheckoutInput): Promise<CardCheckoutOutput> {
    const apiKey = this.config.get<string>('ABACATE_PAY_API_KEY');
    if (!apiKey || apiKey.startsWith('sua_chave')) {
      this.logger.warn(`[MOCK] Checkout de cartão fake para pedido ${input.pedidoId}`);
      return {
        transacaoId: `mockcheckout_${input.pedidoId.slice(0, 8)}_${Date.now()}`,
    ...
    const baseUrl =
      this.config.get<string>('ABACATE_PAY_BASE_URL') ?? 'https://api.abacatepay.com';
    const response = await axios.post(
      `${baseUrl}/v1/checkouts/create`,
      {
        amount: input.valorCentavos,
        methods: ['CARD'],
        card: { maxInstallments: input.parcelas },
        externalId: input.pedidoId,
  ...
  async simulatePayment(
    ...
    const apiKey = this.config.get<string>('ABACATE_PAY_API_KEY');
    if (!apiKey || apiKey.startsWith('sua_chave')) return null;

    const baseUrl =
      this.config.get<string>('ABACATE_PAY_BASE_URL') ?? 'https://api.abacatepay.com';
    const response = await axios.post(
      `${baseUrl}/v1/pixQrCode/simulate-payment`,
      { metadata: {} },
      {
