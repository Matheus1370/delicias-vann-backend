...
describe('PaymentGatewayService', () => {
  ...
  describe('createCardCheckout', () => {
    const baseInput = {
      ...
      returnUrl: 'http://localhost/pedidos/pedido-123',
      completionUrl: 'http://localhost/pedidos/pedido-123',
    };

    it('sem API key retorna mock com checkoutUrl = returnUrl', async () => {
      configService.get.mockImplementation((k: string) =>
        k === 'ABACATE_PAY_API_KEY' ? 'sua_chave_aqui' : undefined,
      );

      const out = await service.createCardCheckout(baseInput);

      expect(out.checkoutUrl).toBe(baseInput.returnUrl);
      expect(out.transacaoId).toMatch(/^mockcheckout_/);
    });
  });
