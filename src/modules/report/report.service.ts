...
export class ReportService {
  ...
  async vendasDiarias(days = 14) {
    ...
    for (let i = days - 1; i >= 0; i--) {
      chaves.push(this.diaLocal(new Date(agora.getTime() - i * 86_400_000)));
    }
    const since = new Date(`${chaves[0]}T00:00:00-03:00`);
    ...
    for (const chave of chaves) {
      buckets.set(chave, { pedidos: 0, faturamento: 0 });
    }

    for (const p of pedidos) {
      const bucket = buckets.get(this.diaLocal(p.createdAt));
      if (bucket) {
        bucket.pedidos += 1;
