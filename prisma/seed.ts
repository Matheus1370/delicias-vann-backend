...
async function main() {
  ...
  const boloPersonalizado = await prisma.produto.upsert({
    ...
    create: {
      ...
      descricao: 'Criado sob medida, vendido por quilo (mínimo 1kg). Massa, recheio e decoração à sua escolha.',
      tipo: 'MONTAVEL',
  ...
  await prisma.opcaoMontagem.deleteMany({
    where: { produtoId: boloPersonalizado.id, etapa: 'tamanho' },
  });
  ...
  await prisma.produto.upsert({
    where: { slug: 'pao-de-mel' },
    update: {},
    create: {
      nome: 'Pão de Mel',
      slug: 'pao-de-mel',
      descricao: 'Massa de mel macia, recheada com brigadeiro e banhada no chocolate.',
      tipo: 'PADRAO',
      fulfillment: 'MAKE_TO_STOCK',
      precoVenda: 12,
      estoqueVitrine: 24,
      ...
      leadTimeHoras: 24,
      categoriaId: cats[1].id,
      imagemUrl: IMG.paodemel,
      alergenicos: ['glúten', 'ovo', 'leite'],
    },
  });

  await prisma.produto.upsert({
    where: { slug: 'torta-de-limao-fatia' },
    update: {},
    create: {
      nome: 'Torta de Limão (fatia)',
      slug: 'torta-de-limao-fatia',
      descricao: 'Base crocante, creme de limão e merengue maçaricado — pronta pra levar.',
      tipo: 'PADRAO',
      fulfillment: 'MAKE_TO_STOCK',
      precoVenda: 15,
      estoqueVitrine: 12,
  ...
  await prisma.produto.upsert({
    where: { slug: 'adicional-docinho-brigadeiro' },
    update: {
      nome: 'Cento de brigadeiro (100 un)',
      descricao: '100 brigadeiros tradicionais enrolados à mão',
      precoVenda: 200,
    },
    create: {
      nome: 'Cento de brigadeiro (100 un)',
      slug: 'adicional-docinho-brigadeiro',
      descricao: '100 brigadeiros tradicionais enrolados à mão',
      tipo: 'ADICIONAL',
      fulfillment: 'MAKE_TO_ORDER',
      precoVenda: 200,
  ...
  await prisma.produto.upsert({
    where: { slug: 'kit-cafe-corporativo' },
    update: {},
    create: {
      nome: 'Kit Café Corporativo (20 pessoas)',
      slug: 'kit-cafe-corporativo',
      descricao: 'Bolos, docinhos e mini tortas pra reunião ou integração do time',
      tipo: 'PADRAO',
      fulfillment: 'MAKE_TO_ORDER',
      precoVenda: 380,
      pontosEsforco: 6,
      leadTimeHoras: 48,
      categoriaId: cats[3].id,
      imagemUrl: IMG.torta,
      alergenicos: ['glúten', 'ovo', 'leite'],
      visivelPJ: true,
    },
  ...
  await prisma.produto.upsert({
    where: { slug: 'kit-cesta-natal' },
    update: {},
    create: {
      nome: 'Cesta de Natal da Vann',
      slug: 'kit-cesta-natal',
      descricao: 'Cesta artesanal com bolo, brigadeiros e cartão personalizado por colaborador',
      tipo: 'PADRAO',
      fulfillment: 'MAKE_TO_ORDER',
      precoVenda: 145,
      pontosEsforco: 3,
  ...
  const opcoes: Array<Prisma.OpcaoMontagemCreateInput> = [
    ...
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'massa', label: 'Red Velvet', descricao: 'edição especial · descansa 24h', precoExtra: 0, precoExtraPorKg: 15, pontosExtra: 1, leadTimeHorasExtra: 24, ordem: 3 },
    ...
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Doce de Leite', descricao: 'puro aconchego', precoExtra: 0, precoExtraPorKg: 8, pontosExtra: 0, ordem: 3 },
    ...
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'massa', label: 'Branco', descricao: 'leve e fofinha', precoExtra: 0, pontosExtra: 0, ordem: 4 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'massa', label: 'Cenoura', descricao: 'com calda de chocolate', precoExtra: 0, pontosExtra: 0, ordem: 5 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'massa', label: 'Milho', descricao: 'cremosa, lembrança de festa junina', precoExtra: 0, pontosExtra: 0, ordem: 6 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'massa', label: 'Frutas', descricao: 'com frutas cristalizadas', precoExtra: 0, pontosExtra: 0, ordem: 7 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'massa', label: 'Pistache', descricao: 'sofisticada · importada', precoExtra: 0, precoExtraPorKg: 18, pontosExtra: 0, ordem: 8 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Ninho', descricao: 'leitinho Ninho cremoso', precoExtra: 0, pontosExtra: 0, ordem: 4 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Nutella', descricao: 'irresistível', precoExtra: 0, precoExtraPorKg: 12, pontosExtra: 0, ordem: 5 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Ovomaltine', descricao: 'crocante de verdade', precoExtra: 0, pontosExtra: 0, ordem: 6 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Trufa', descricao: 'chocolate intenso', precoExtra: 0, precoExtraPorKg: 10, pontosExtra: 0, ordem: 7 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Coco', descricao: 'beijinho de festa', precoExtra: 0, pontosExtra: 0, ordem: 8 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Amendoim', descricao: 'paçoca cremosa', precoExtra: 0, pontosExtra: 0, ordem: 9 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Cream Cheese', descricao: 'levemente azedinho', precoExtra: 0, pontosExtra: 0, ordem: 10 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Lemon Curd', descricao: 'creme de limão siciliano', precoExtra: 0, pontosExtra: 0, ordem: 11 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Frutas Vermelhas', descricao: 'geleia artesanal', precoExtra: 0, pontosExtra: 0, ordem: 12 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Cereja', descricao: 'doce e marcante', precoExtra: 0, pontosExtra: 0, ordem: 13 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Canela', descricao: 'puro aconchego', precoExtra: 0, pontosExtra: 0, ordem: 14 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Mousse Maracujá', descricao: 'azedinho refrescante', precoExtra: 0, pontosExtra: 0, ordem: 15 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Pistache', descricao: 'delicado e sofisticado', precoExtra: 0, precoExtraPorKg: 15, pontosExtra: 0, ordem: 16 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Chantilly', descricao: 'leve e aerado', precoExtra: 0, pontosExtra: 0, ordem: 17 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'cobertura', label: 'Buttercream', descricao: 'versátil, aceita qualquer cor', precoExtra: 0, pontosExtra: 0, ordem: 4 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'cobertura', label: 'Cream Cheese', descricao: 'par perfeito do red velvet', precoExtra: 0, pontosExtra: 0, ordem: 5 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'cobertura', label: 'Doce de Leite', descricao: 'brilho caramelado', precoExtra: 0, pontosExtra: 0, ordem: 6 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'cobertura', label: 'Ninho', descricao: 'delicada e docinha', precoExtra: 0, pontosExtra: 0, ordem: 7 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'cobertura', label: 'Paçoca', descricao: 'rústica e marcante', precoExtra: 0, pontosExtra: 0, ordem: 8 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'recheio', label: 'Sem recheio', descricao: 'massa pura, sem recheio', precoExtra: 0, pontosExtra: 0, ordem: 0 },
    { produto: { connect: { id: boloPersonalizado.id } }, etapa: 'cobertura', label: 'Sem cobertura', descricao: 'bolo nu, sem cobertura por cima', precoExtra: 0, pontosExtra: 0, ordem: 0 },
  ];
    ...
    if (existente) {
      await prisma.opcaoMontagem.update({
        ...
        data: {
          ...
          precoExtraPorKg: op.precoExtraPorKg ?? 0,
  ...
  if ((await prisma.fotoProduto.count()) === 0) {
    const fatia = 'https://images.unsplash.com/photo-1571115177098-24ec42ed204d?w=600';
    await prisma.fotoProduto.createMany({
      data: [
        { produtoId: boloPersonalizado.id, url: IMG.bolo, tipo: 'PRINCIPAL', ordem: 1 },
        { produtoId: boloPersonalizado.id, url: fatia, tipo: 'CORTADO', ordem: 2 },
        { produtoId: boloPersonalizado.id, url: IMG.torta, tipo: 'DETALHE', ordem: 3 },
        { produtoId: boloPote.id, url: IMG.bolopote, tipo: 'PRINCIPAL', ordem: 1 },
        { produtoId: caixaBrigadeiros.id, url: IMG.brigadeiro, tipo: 'PRINCIPAL', ordem: 1 },
        { produtoId: caixaBrigadeiros.id, url: IMG.torta, tipo: 'DETALHE', ordem: 2 },
      ],
    });
  }

  if ((await prisma.bastidorPost.count()) === 0) {
    await prisma.bastidorPost.createMany({
      data: [
        { imagemUrl: 'https://images.unsplash.com/photo-1556910103-1c02745aae4d?w=600', legenda: 'a cozinha já tá quentinha desde cedo 🥄', ordem: 1, ativo: true },
        { imagemUrl: 'https://images.unsplash.com/photo-1486427944299-d1955d23e34d?w=600', legenda: 'fornada de hoje saindo pra festa 💖', ordem: 2, ativo: true },
        { imagemUrl: IMG.brigadeiro, legenda: 'brigadeiro enrolado um a um, sem pressa', ordem: 3, ativo: true },
        { imagemUrl: IMG.torta, legenda: 'aquele red velvet que vocês amam 🍰', ordem: 4, ativo: true },
      ],
    });
  }
